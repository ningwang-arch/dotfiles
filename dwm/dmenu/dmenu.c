...
static void drawmenu(void) {
    ...
    drw_text_align(
        drw, x + curpos, 0, w - curpos, bh, text + cursor, strlen(text) - cursor, AlignL);
...
static void keypress(XKeyEvent* ev) {
    ...
    if (ev->state & ControlMask) {
        ...
        case XK_Y:
            XConvertSelection(
                dpy, (ev->state & ShiftMask) ? clip : XA_PRIMARY, utf8, utf8, win, CurrentTime);
            return;
        case XK_Left:
        case XK_KP_Left:
            movewordedge(-1);
...
static void readstdin(void) {
    ...
    for (i = 0; (len = getline(&line, &junk, stdin)) != -1; i++, line = NULL) {
        if (i + 1 >= size / sizeof *items)
            if (!(items = realloc(items, (size += BUFSIZ)))) die("cannot realloc %zu bytes:", size);
        if (line[len - 1] == '\n') line[len - 1] = '\0';
        items[i].text = line;
        items[i].out = 0;
    }
