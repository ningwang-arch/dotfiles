...
Atom getatomprop(Client* c, Atom prop) {
    ...
    if (XGetWindowProperty(
            dpy, c->win, prop, 0L, sizeof atom, False, req, &da, &di, &dl, &dl, &p) == Success &&
        p) {
        atom = *(Atom*)p;
...
void fullscreen(const Arg* arg) {
    if (selmon->showbar) {
        for (last_layout = (Layout*)layouts; last_layout != selmon->lt[selmon->sellt];
             last_layout++)
            ;
    ...
    for (i = 0; i < LENGTH(tags); i++) {
        if (selmon->tagset[selmon->seltags] & 1 << i) {
            ...
            if (occ & 1 << i) {
                ...
                // uncomment the following line and comment the other imlin_copy.. line if you don't
                // want the bar showing on the preview imlib_copy_drawable_to_image(0, selmon->wx,
                // selmon->wy, selmon->ww ,selmon->wh, 0, 0, 1);
                imlib_copy_drawable_to_image(
                    0, selmon->mx, selmon->my, selmon->mw, selmon->mh, 0, 0, 1);
                ...
                imlib_render_image_part_on_drawable_at_size(0,
                                                            0,
                                                            selmon->mw,
                                                            selmon->mh,
                                                            0,
    ...
    for (i = 0, my = ty = m->gappoh * oe, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
        if (i < m->nmaster) {
            ...
            resize(c,
                   ...
                   h - (2 * c->bw),
                   0);
            if (my + HEIGHT(c) < m->wh) my += HEIGHT(c) + m->gappih * ie;
        }
        else {
            r = n - i;
            ...
            resize(c,
                   ...
                   h - (2 * c->bw),
                   0);
            if (ty + HEIGHT(c) < m->wh) ty += HEIGHT(c) + m->gappih * ie;
        }
}
...
void updatesystray(void) {
    ...
    wc.sibling = m->barwin;
    XConfigureWindow(
        dpy, systray->win, CWX | CWY | CWWidth | CWHeight | CWSibling | CWStackMode, &wc);
...
void shiftviewclients(const Arg* arg) {
    ...
    for (c = selmon->clients; c; c = c->next) tagmask |= c->tags;
