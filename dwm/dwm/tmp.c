...
void buttonpress(XEvent* e) {
    ...
    for (i = 0; i < LENGTH(buttons); i++)
        if (click == buttons[i].click && buttons[i].func && buttons[i].button == ev->button &&
            ...
            buttons[i].func((click == ClkTagBar || click == ClkWinTitle) && buttons[i].arg.i == 0
                                ? &arg
                                : &buttons[i].arg);
        }
}
...
void incnmaster(const Arg* arg) {
    ...
    if (selmon->pertag->curtag == 0) { selmon->pertag->nmasters[0] = selmon->nmaster; }
    arrange(selmon);
}
...
void resizeclient(Client* c, int x, int y, int w, int h) {
    ...
    if (c->isfloating || c->mon->lt[c->mon->sellt]->arrange == NULL) { gapincr = gapoffset = 0; }
    else {
...
void setmfact(const Arg* arg) {
    ...
    if (selmon->pertag->curtag == 0) { selmon->pertag->mfacts[0] = f; }
    arrange(selmon);
}
...
void togglebar(const Arg* arg) {
    ...
    if (selmon->pertag->curtag == 0) { selmon->pertag->showbars[0] = selmon->showbar; }
    updatebarpos(selmon);
