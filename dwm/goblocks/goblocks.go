...
func setStyle(style string) []string {
	...
	return []string{
                briefStyle + netColor + "^"+ updateNet(),
		briefStyle + cpuColor + "^"+ updateCPU(),
		briefStyle + memColor + "^"+ updateMem(),
		briefStyle + volColor + "^"+ updateVolume(),
		briefStyle + ligColor + "^"+ updateBright(),
		briefStyle + batColor + "^"+ updateBattery(),
		briefStyle + datColor + "^"+ updateDateTime(),
	}
}
...
func fmtNetSpeed(speed float64) string {
	...
	case speed >= (1024 * 1024 * 1024):
		...
		res = fmt.Sprintf("%.2f", gbSpeed) + "Gb/s"
	case speed >= (1024 * 1024):
		mbSpeed := speed / (1024.0 * 1024.0)
		res = fmt.Sprintf("%.1f", mbSpeed) + "Mb/s"
	case speed >= 1024:
		kbSpeed := speed / 1024.0
		res = fmt.Sprintf("%.1f", kbSpeed) + "kb/s"
	case speed >= 0:
		res = fmt.Sprint(speed) + "B/s"
	}
...
func updateBright() string{
	const light = "light"
	brt_str :=cmdReturn(light, "-G")
	brightness := brt_str[0:len(brt_str)-3]
	return "ﯧ "+brightness+"%"
}
...
func updateBattery() string {
	...
	var pathToADP0 = pathToPowerSupply + "ADP0/"

	status := parseTxt(pathToBat0, "status")
	capacity := parseTxt(pathToBat0, "capacity")
	isPlugged, _ := strconv.ParseBool(parseTxt(pathToADP0, "online"))
	if status == "Full" {
...
func parseConfig() {
  ...
  volColor = config.Get("color.volColor").(string)
	ligColor = config.Get("color.ligColor").(string)
...
func updateDateTime() string {
	...
	var dateTime = time.Now().Local().Format("01/02/06 Mon 15:04 ")
