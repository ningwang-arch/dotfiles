...
void xclear(int x1, int y1, int x2, int y2) {
    XftDrawRect(
        xw.draw, &dc.col[IS_SET(MODE_REVERSE) ? defaultfg : defaultbg], x1, y1, x2 - x1, y2 - y1);
}
...
int xloadfont(Font* f, FcPattern* pattern) {
    ...
    XftTextExtentsUtf8(
        xw.dpy, f->match, (const FcChar8*)ascii_printable, strlen(ascii_printable), &extents);
...
void xinit(int cols, int rows) {
    ...
    XChangeProperty(
        xw.dpy, xw.win, xw.netwmpid, XA_CARDINAL, 32, PropModeReplace, (uchar*)&thispid, 1);
...
void xdrawglyphfontspecs(const XftGlyphFontSpec* specs, Glyph base, int len, int x, int y) {
    ...
    if (base.mode & ATTR_UNDERLINE) {
        ...
        if (base.ustyle != 3) {
            ...
            XFillRectangle(
                xw.dpy, XftDrawDrawable(xw.draw), ugc, winx, winy + dc.font.ascent + 1, width, wlw);
        }
            ...
            */
            ...
            if (npoints >= 3) {
                ...
                // Draw the lines
                XDrawLines(
                    xw.dpy, XftDrawDrawable(xw.draw), ugc, points, npoints, CoordModePrevious);
        ...
        // Cap is half of wave width
            ...
            if (npoints >= 3) {
                ...
                // Draw a second underline with an offset of 1 pixel
                if (((win.ch / (widthThreshold / 2)) % 2)) {
                    for (int i = 0; i < npoints; i++) points[i].x++;

                    XDrawLines(
                        xw.dpy, XftDrawDrawable(xw.draw), ugc, points, npoints, CoordModeOrigin);
                }
...
int xstartdraw(void) {
    if (IS_SET(MODE_VISIBLE)) XCopyArea(xw.dpy, xw.win, xw.buf, dc.gc, 0, 0, win.w, win.h, 0, 0);
