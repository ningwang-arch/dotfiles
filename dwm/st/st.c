...
void tscrollup(int orig, int n, int copyhist) {
    ...
    if (term.scr > 0 && term.scr < HISTSIZE) term.scr = MIN(term.scr + n, HISTSIZE - 1);
...
void readcolonargs(char** p, int cursor, int params[][CAR_PER_ARG]) {
    int i = 0;
    for (; i < CAR_PER_ARG; i++) params[cursor][i] = -1;
...
void osc4_color_response(int num) {
    ...
    n = snprintf(
        buf, sizeof buf, "\033]4;%d;rgb:%02x%02x/%02x%02x/%02x%02x\007", num, r, r, g, g, b, b);
    ...
    */
    while (passes != term.bot + 2) {
        ...
        */
        ...
        if ((match = findlastany(linestr, URLSTRINGS, sizeof(URLSTRINGS) / sizeof(URLSTRINGS[0]))))
            break;

        if (--row < term.top) row = term.bot;
